...
export const JobDetailsBriefCard = () => {
	// default - when not open
	// brief - when the brief is shown
	// jobApplication - when the job application is shown
	const [openStatus, setOpenStatus] = useState<
		"default" | "brief" | "jobApplication"
	>("default");
	...
	return (
		<Card className="grow border-none">
			...
			<Dialog
				open={openStatus === "brief"}
				onOpenChange={(open) => {
					if (open) {
						return setOpenStatus("brief");
					}
						...
						<div className="flex justify-between items-center gap-x-4 w-full">
							...
							<div className="pr-3">
								<Button
									...
									onClick={() => setOpenStatus("jobApplication")}>
									Easy Apply
								</Button>
							</div>
						</div>
			...
			<ApplyToJob.Provider
				open={openStatus === "jobApplication"}
				onOpenChange={(open) =>
